def build_generator(input_var=None):
    ...
    network = TransposedConv2DLayer(network, num_filters=128, filter_size=(4, 4),
                                                   stride=(1, 1), W=lasagne.init.Normal(std=0.02, mean=0.0), nonlinearity=rectify)

    network = TransposedConv2DLayer(network, num_filters=128/2,
              filter_size=(5, 5), stride=(2, 2), crop=2, W=lasagne.init.Normal(std=0.02, mean=0.0), nonlinearity=rectify, output_size = 8)

    network = TransposedConv2DLayer(network, num_filters=128/4,
              filter_size=(5, 5), stride=(2, 2), crop=2, W=lasagne.init.Normal(std=0.02, mean=0.0), nonlinearity=rectify, output_size = 16)

    network = TransposedConv2DLayer(network, num_filters=3,
                                                   filter_size=(5, 5), stride=(2, 2), crop=2, W=lasagne.init.Normal(std=0.02, mean=0.0),
...
def train(num_epochs=1,
          lr=0.001, example=19, save_freq=100,
...
#     target = T.tensor4('targets',dtype='float32')
    ...
    discriminator_loss = (lasagne.objectives.squared_error(real_out, b).mean() +
    ...
    # Generate some samples 
        if epoch == 1:
            generate_and_show_sample(generate_sample_fn, nb=example, seed=i)
