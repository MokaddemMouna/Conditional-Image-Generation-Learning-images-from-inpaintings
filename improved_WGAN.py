...
def train(num_epochs=1, epochsize=5, clip=0.01,
          lr=0.0001, LAMBDA=10, example=19, save_freq=100,
...
#     target = T.tensor4('targets',dtype='float32')
    ...
    epsilon = np.random.uniform(low=0.0, high=1.0, size=(batch_size,32,32,3))
    
    # Calculating the gradient penalty which is the improving part added by the article
    x_hat = sample + epsilon*(input_var - sample)
    print "mouna"
    critic_interpol = lasagne.layers.get_output(build_discriminator(x_hat))
    print "mouna"
    gradients = T.jacobian(critic_interpol,x_hat)
    print "mouna"
    slopes = T.sqrt((T.square(gradients)).sum(1))
    gradient_penalty = ((slopes-1.)**2).mean()
    critic_score = critic_score + LAMBDA* gradient_penalty
    
#     differences = real_out - fake_out
#     print "mouna"
#     interpolates = fake_out + (epsilon*differences)
#     print "mouna"
#     critic_interpol = lasagne.layers.get_output(build_discriminator(interpolates))
#     print "mouna"
#     gradients = T.grad(critic_interpol,interpolates)
#     slopes = T.sqrt((T.square(gradients)).sum(1))
#     gradient_penalty = ((slopes-1.)**2).mean()
#     critic_score = critic_score + LAMBDA* gradient_penalty
    ...
    generator_updates = lasagne.updates.adam(-generator_score, generator_params, learning_rate=lr, 
                                             beta1=0.5, beta2=0.9)
    
    critic_updates = lasagne.updates.adam(critic_score, critic_params, learning_rate=lr,
                                         beta1=0.5, beta2=0.9)
